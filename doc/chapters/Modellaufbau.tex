...
[T_{\min},T_{\max}] \\ 
[C_{L, \min},C_{L, \max}]
\end{matrix} \right]\right) \subset \R^2 & &(U : [t_0,t_f] \to \R^m)
\end{align*}
...
\end{pmatrix} \stackrel{!}{\geq} 0\] wobei \[\sigma(x(t),\lambda(t)) := H_u(X^{\ast}(t),U^{\ast}(t),\lambda(t))\] Schaltfunktion genannt wird. Für die Betrachtung mit den Beschränkungen der Steuerfunktion ergibt sich:
\[T(t) = \left\lbrace \begin{array}{ll}
T_{\min} & ,\text{falls } \lambda_4 > 0  \\ 
\text{beliebig} & ,\text{falls } \lambda_4 = 0  \\ 
T_{\max} & ,\text{falls } \lambda_4 < 0
\end{array} \right.\]
und 
\[C_L(t) = \left\lbrace \begin{array}{ll}
T_{\max} & ,\text{falls } \lambda_4 < 0 \\ 
 & ,\text{falls } \lambda_4 = 0  \\ 
T_{\min} & ,\text{falls } \lambda_4 > 0 
\end{array} \right.\]
%
...
\dfrac{\partial}{\partial \gamma} H &= \cos(\gamma(t)) v(t) \lambda_1(t) + \dfrac{g \sin(\gamma(t)) \lambda_2(t)}{v(t)} - \dfrac{\sin(\gamma(t)) v^2(t) \lambda_3(t)}{v(t)} - \cos(\gamma(t)) \lambda_4 g(t)\\\
