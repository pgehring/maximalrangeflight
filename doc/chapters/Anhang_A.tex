...
\begin{table}[H]
    ...
    \end{tabularx}
\end{table}
Für die Ergebnisse von Versuch 0.1 (Kapitel \ref{kap:Versuch01}) und Versuch 0.2 (Kapitel \ref{kap:Versuch02}) wird der folgende technische Aufwand (Tabelle \ref{tab:Versuch0_TA}) benötigt.
\begin{table}[H]
    \centering
...
In Versuch 0.1 wird das Ergebnis in Abbildung \ref{img:test_0_1} erreicht. Dies stellt aufgrund der Exit Flag $-2$ kein Optimum des Problems da. 
\begin{figure}[H]
\begin{center}
...
In Versuch 0.1 wird das Ergebnis in Abbildung \ref{img:test_0_2} erreicht. Dies stellt aufgrund der Exit Flag $-2$ kein Optimum des Problems da.
\begin{figure}[H]
\begin{center}
...
Die Problemstellung \ref{prob:MaxRF} gibt den Parametern {tab:ProblemPara} eine feste Endzeit $t_f = 1800 \ s$ beziehungsweise einen Zeitraum vor in welchem das Problem gelöst werden soll. In diesem Versuch wird diese nun verkürzt, auf $t_f = 300 \ s$ und $t_f = 350 \ s$. Für die beiden Versuche werden die folgenden Einstellungen aus Tabelle \ref{tab:Versuch1} verwendet.
\begin{table}[H]
    \centering
    ...
    \end{tabularx}
\end{table}
Für die Ergebnisse von Versuch 1.1 (Kapitel \ref{kap:Versuch11}) und Versuch 1.2 (Kapitel \ref{kap:Versuch12}) wird der folgende technische Aufwand (Tabelle \ref{tab:Versuch1_TA}) benötigt.
\begin{table}[H]
    \centering
...
In Versuch 1.1 wird das Ergebnis in Abbildung \ref{img:test_1_1} erreicht. Dies stellt aufgrund der Exit Flag $2$ ein mögliches Optimum des Problems da, da die Änderung des Zustandsvektors $z$ kleiner ist die gegebene Toleranz.
\begin{figure}[H]
\begin{center}
...
In Versuch 1.2 wird das Ergebnis in Abbildung \ref{img:test_1_2} erreicht. Dies stellt aufgrund der Exit Flag $2$ ein mögliches Optimum des Problems da, da die Änderung des Zustandsvektors $z$ kleiner ist die gegebene Toleranz. Die Unstetigkeiten im Verlauf des Anstellwinkels und der Geschwindigkeit deuten jedoch auf kein physikalisch plausibles Ergebnis hin.
\begin{figure}[H]
\begin{center}
...
\begin{table}[H]
    ...
    \end{tabularx}
\end{table}
Für die Ergebnisse von Versuch 2.1 (Kapitel \ref{kap:Versuch21}) und Versuch 2.2 (Kapitel \ref{kap:Versuch22}) wird der folgende technische Aufwand (Tabelle \ref{tab:Versuch2_TA}) benötigt.
\begin{table}[H]
    \centering
...
In Versuch 2.1 wird das Ergebnis in Abbildung \ref{img:test_2_1} erreicht. Dies stellt aufgrund der Exit Flag $-2$ kein Optimum des Problems da.
\begin{figure}[H]
\begin{center}
...
In Versuch 2.2 wird das Ergebnis in Abbildung \ref{img:test_1_2} erreicht. Dies stellt aufgrund der Exit Flag $2$ ein mögliches Optimum des Problems da, da die Änderung des Zustandsvektors $z$ kleiner ist die gegebene Toleranz. Die Unstetigkeiten im Verlauf des Anstellwinkels und der Geschwindigkeit deuten jedoch auf kein physikalisch plausibles Ergebnis hin.
\begin{figure}[H]
\begin{center}
...
Zusätzlich zur verkürzten Endzeit wie in Versuch 1 (Kapitel \ref{kap:Versuch1}) wird nun zusätzlich das Gewicht des Flugzeuges angepasst. Das maximale Startgewicht eines A380-800 beträgt max. $560.000 \ [kg]$ \cite{A380Tech}. Gewählt wird eine Masse von $500.000 \ [kg]$. Für die beiden Versuche werden die folgenden Einstellungen aus Tabelle \ref{tab:Versuch3} verwendet.
\begin{table}[H]
    \centering
    ...
    \end{tabularx}
\end{table}
Für die Ergebnisse von Versuch 3.1 (Kapitel \ref{kap:Versuch31}) und Versuch 3.2 (Kapitel \ref{kap:Versuch32}) wird der folgende technische Aufwand (Tabelle \ref{tab:Versuch3_TA}) benötigt.
\begin{table}[H]
    \centering
    ...
    \begin{tabularx}{.9\textwidth}{Zccc}
        ...
        \midrule
        Funktionswert der Zielfunktion & $-102713.3569$ & $-140202.4068$ \\
        Anzahl Iterationen & $12970$ & $6432$ \\
        Anzahl Funktionsauswertungen & $15617295$ & $7740311$ \\
        Exit Flag von \textit{MATLAB} & $2$ & $2$ \\
        Optimalität des Ergebnis & $0.021006$ & $0.0043122$ \\
        Berechnungsdauer & $334.723 \ min$ & $251.0802 \ min$ \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[H]
    ...
    \end{tabularx}
\end{table}
Für die Ergebnisse von Versuch 4.1 (Kapitel \ref{kap:Versuch41}) und Versuch 4.2 (Kapitel \ref{kap:Versuch42}) wird der folgende technische Aufwand (Tabelle \ref{tab:Versuch4_TA}) benötigt.
\begin{table}[H]
    \centering
...
In Versuch 4.1 wird das Ergebnis in Abbildung \ref{img:test_4_1} erreicht. Dies stellt aufgrund der Exit Flag $2$ ein mögliches Optimum des Problems da, da die Änderung des Zustandsvektors $z$ kleiner ist die gegebene Toleranz.
\begin{figure}[H]
\begin{center}
...
In Versuch 4.2 wird das Ergebnis in Abbildung \ref{img:test_4_2} erreicht. Dies stellt aufgrund der Exit Flag $2$ ein mögliches Optimum des Problems da, da die Änderung des Zustandsvektors $z$ kleiner ist die gegebene Toleranz. Bei diesem Versuch ist zu beobachten, dass mit der möglichen Steuerung die physikalische Grenze der maximal Geschwindigkeit erreicht wird. Diese wird dann auf maximalem Niveau bis zum Endpunkt gehalten, um eine maximale Geschwindigkeit zu erreichen.
\begin{figure}[H]
\begin{center}
